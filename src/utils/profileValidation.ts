...
export const isProfileComplete = (user: User | null): boolean => {
  ...
  const hasBasicInfo = user.name && user.name.trim().length > 0;
  const hasSkillsOffered = user.skillsOffered && user.skillsOffered.length > 0;
  const hasSkillsWanted = user.skillsWanted && user.skillsWanted.length > 0;
  const hasAvailability = user.availability && user.availability.trim().length > 0;
